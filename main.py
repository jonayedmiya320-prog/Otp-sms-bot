...
# ─── Default Panels (panels.json না থাকলে এগুলো চলবে) ───
DEFAULT_PANELS = [
    {"url": "http://139.99.69.196",         "username": "Waleedbhai",  "password": "Waleedbhai"},
    {"url": "https://www.konektapremium.net","username": "spyxrana",   "password": "spyxrana"},
]
...
class MasdarAlkonOTPBot:
    ...
    async def auto_login_with(self, url, username, password):
        """নির্দিষ্ট credential দিয়ে login করুন — Masdar, Konekta, TimeSMS সব support"""
        try:
            ...
            is_timesms = 'timesms' in url.lower()

            if is_konekta:
                return await self._login_konekta(url, username, password)
            elif is_timesms:
                return await self._login_timesms(url, username, password)
            else:
    ...
    async def _login_timesms(self, url, username, password):
        """TimeSMS panel login — /login endpoint"""
        try:
                ...
                if capt_input:
                    ...
                    if parent:
                        ...
                        if len(numbers) >= 2:
                            ...
                            LOGGER.info(f"🧮 TimeSMS Captcha: {captcha_answer}")
            ...
            async with self.session.post(
                ...
                if 'login' not in final_url.lower():
                    LOGGER.info(f"🎉 TimeSMS login সফল: {url}")
                    self.last_login_time = time.time()
                    return True
                LOGGER.error(f"❌ TimeSMS login ব্যর্থ: {final_url}")
                return False

        except Exception as e:
            LOGGER.error(f"❌ TimeSMS login error: {e}")
            return False
    ...
    async def get_sms_data_api(self):
        ...
        try:
            ...
            is_timesms = 'timesms' in self.base_url.lower()
            ...
            if is_konekta:
                ...
                api_url = (
                    f"{self.base_url}/agent/res/data_creditnotes.php?"
                    f"fdate1={today}%2000:00:00&fdate2={today}%2023:59:59&"
                    f"sEcho=1&iColumns=7&sColumns=%2C%2C%2C%2C%2C%2C&"
                    f"iDisplayStart=0&iDisplayLength=100&"
                    f"mDataProp_0=0&sSearch_0=&bRegex_0=false&bSearchable_0=true&bSortable_0=true&"
                ...
                referer  = f"{self.base_url}/agent/CreditNotes"
                use_ssl  = True
            else:
                api_url = (
